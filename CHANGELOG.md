...
- Integrated `CA\Log` (`groupesti/laravel-ca-log`) for structured logging across all certificate operations.
- Certificate issuance (from CSR, self-signed, intermediate) is now logged via `CaLog::certificateIssued()` with serial, subject, and context.
- Certificate renewal is logged via `CaLog::log('certificate_renewal', ...)` with old and new serial numbers.
- Certificate revocation is logged via `CaLog::certificateRevoked()` with serial and reason.
- Certificate validation failures are logged via `CaLog::log('certificate_validation', 'warning', ...)` with error details.
- All exceptions in signing, export, chain building, issuance, renewal, and revocation are caught, logged via `CaLog::critical()`, and re-thrown.
