...
final class CertificateRenewer
{
    ...
    public function renew(Certificate $certificate, ?int $validityDays = null): Certificate
    {
        try {
            ...
            CaLog::log('certificate_renewal', 'info', "Certificate renewed: {$newCertificate->serial_number}", [
                'old_serial' => $certificate->serial_number,
                'new_serial' => $newCertificate->serial_number,
                'ca_id' => $certificate->ca_id,
                'validity_days' => $days,
                'renewed_from' => $certificate->uuid,
            ]);
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'certificate_renewal', 'exception' => $e::class]);
