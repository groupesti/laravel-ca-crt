...
final class CertificateManager implements CertificateManagerInterface
{
    ...
    public function issueFromCsr(
        ...
        try {
            ...
            CaLog::certificateIssued($certificate->serial_number, json_encode($csr->subject_dn), [
                'ca_id' => $ca->id,
                'csr_id' => $csr->id,
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'certificate_issuance', 'exception' => $e::class]);
    ...
    public function issueSelfSigned(
        ...
        try {
            ...
            CaLog::certificateIssued($certificate->serial_number, json_encode($subjectDn), [
                'ca_id' => $ca->id,
                'type' => $options->type->value,
                'issued_from' => 'self_signed',
                'validity_days' => $options->validityDays,
    ...
    public function issueIntermediate(
        ...
        try {
            ...
            CaLog::certificateIssued($certificate->serial_number, json_encode($dn->toArray()), [
                'ca_id' => $parentCa->id,
                'type' => CertificateType::INTERMEDIATE_CA->value,
                'issued_from' => 'intermediate',
                'parent_ca_id' => $parentCa->id,
                'validity_days' => $options->validityDays,
                'fingerprint' => $certificate->fingerprint_sha256,
    ...
    public function revoke(Certificate $certificate, RevocationReason $reason): Certificate
    {
        try {
            ...
            CaLog::certificateRevoked($certificate->serial_number, $reason->name, [
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'certificate_revocation', 'exception' => $e::class]);
